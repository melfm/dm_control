...
class JacoArm(base.RobotArm):
    ...
    def _add_velocity_actuator(self, joint):
        ...
        if joint.name in self._large_joints:
            ...
            raise ValueError(
                '`joint.name` must be one of {}, got {!r}.'.format(
                    self._all_joints, joint.name))
        return joint.root.actuator.add('velocity',
                                       joint=joint,
                                       name=joint.name,
                                       kv=vel_gain,
                                       ctrllimited=True,
...
class JacoArmObservables(base.JointsObservables):
    ...
    def joints_torque(self):
        ...
        def get_torques(physics):
            torques = physics.bind(
                self._entity.joint_torque_sensors).sensordata
